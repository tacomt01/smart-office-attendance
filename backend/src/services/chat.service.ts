...
import { extractWorkMinutes } from './parser.service.js';
...
- "เวลาเข้างาน / ชั่วโมงทำงาน / เวลาทำงาน" = ระยะเวลาทำงานจริง = เวลาออก − เวลาเข้า (เป็นชั่วโมง-นาที) นับเฉพาะวันที่สแกนครบทั้งเข้าและออก ส่วนวันที่ไม่สแกนเข้า/ไม่สแกนออก/ขาดงาน/วันหยุด ไม่นำมานับ
  - "เวลาเข้างานมากที่สุด / ดีที่สุด / ทำงานเยอะที่สุด" = ชั่วโมงทำงานรวมมากที่สุด
  - "เวลาเข้างานน้อยที่สุด / ทำงานน้อยที่สุด" = ชั่วโมงทำงานรวมน้อยที่สุด
  - ถ้ามีส่วน "ชั่วโมงทำงานจริง" ที่คำนวณมาให้แล้วใน context ให้ใช้ตัวเลขนั้นตอบทันที ห้ามคำนวณเอง
- ถ้า context ระบุ "ขอบเขตข้อมูล" เป็นช่วงวันที่ ให้ตอบโดยอ้างอิงเฉพาะช่วงวันที่นั้น และบอกช่วงวันที่ในคำตอบด้วย
...
async function getAggregatedData(fullName?: string, dateWhere: Record<string, any> = {}) {
  const where: any = { status: { not: 'holiday' }, ...dateWhere };
...
// ── ชั่วโมงทำงานจริง (เวลาออก − เวลาเข้า เฉพาะวันสแกนครบ) ──
export interface WorkDurationRow {
  name: string;
  totalMin: number; // นาทีรวม
  days: number;     // จำนวนวันที่นับได้ (สแกนครบ)
  avgMin: number;   // เฉลี่ยต่อวัน (นาที)
}
...
/** จัดอันดับชั่วโมงทำงานรวมต่อคน (มาก→น้อย) จาก records ที่สแกนครบ — pure, เทสต์ได้ */
export function computeWorkDurationRanking(
  records: { fullName: string; rawValue: string }[],
): WorkDurationRow[] {
  const agg = new Map<string, { totalMin: number; days: number }>();
  for (const r of records) {
    const worked = extractWorkMinutes(r.rawValue);
    if (worked === null) continue; // ข้ามวันไม่สแกนครบ/ขาด/หยุด
    const cur = agg.get(r.fullName) ?? { totalMin: 0, days: 0 };
    cur.totalMin += worked;
    cur.days += 1;
    agg.set(r.fullName, cur);
  }
  return [...agg.entries()]
    .map(([name, v]) => ({
      name,
      totalMin: v.totalMin,
      days: v.days,
      avgMin: v.days > 0 ? Math.round(v.totalMin / v.days) : 0,
    }))
    .sort((a, b) => b.totalMin - a.totalMin || a.name.localeCompare(b.name));
}

async function getWorkDurationStats(
  fullName?: string,
  dateWhere: Record<string, any> = {},
): Promise<WorkDurationRow[]> {
  // เฉพาะวันที่สแกนครบทั้งเข้า-ออก (normal/late/early_leave) — absent/missing/holiday ไม่มีเวลาให้คำนวณ
  const where: any = { status: { in: ['normal', 'late', 'early_leave'] }, ...dateWhere };
  if (fullName) where.fullName = fullName;
  const records = await prisma.attendanceLog.findMany({
    where,
    select: { fullName: true, rawValue: true },
  });
  return computeWorkDurationRanking(records);
}
...
${rankingSection}${workSection}${historyText}## คำถาม
${userMessage}
...
// คีย์เวิร์ดตรวจจับเจตนา "ถามชั่วโมง/เวลาทำงานจริง" → ฉีดอันดับชั่วโมงทำงานเข้า context
const WORK_DURATION_KEYWORDS = [
  'เวลาเข้างาน',
  'ชั่วโมงทำงาน',
  'ชั่วโมง',
  'กี่ชั่วโมง',
...
// แปลงช่วงวันที่เป็น Prisma where (gte/lte) — คืน {} ถ้าไม่มีช่วง
function buildDateWhere(dateFrom?: string, dateTo?: string): Record<string, any> {
...
// สร้าง section "ชั่วโมงทำงานจริง" สำหรับฉีดเข้า context (Top 15 พอสำหรับคุม token)
function buildWorkSection(rows: WorkDurationRow[]): string {
  if (rows.length === 0) {
    return `## ชั่วโมงทำงานจริง (เฉพาะวันสแกนครบทั้งเข้า-ออก)
ไม่มีข้อมูลวันที่สแกนครบในช่วงที่ระบุ
...
`;
  ...
  const lines = rows
    ...
    .map(
      (r, i) =>
        `${i + 1}. ${r.name} — รวม ${minToHHMM(r.totalMin)} ชม. (${r.days} วัน, เฉลี่ย ${minToHHMM(r.avgMin)}/วัน)`,
    );
  return `## ชั่วโมงทำงานจริง (เฉพาะวันสแกนครบทั้งเข้า-ออก; วันไม่สแกน/ขาด/หยุด ไม่นับ)
อันดับชั่วโมงทำงานรวม มาก→น้อย (บนสุด = ทำงานมาก/ดีที่สุด, ล่างสุด = น้อยที่สุด):
${lines.join('\n')}
...
export async function chat(
  ...
  // กรองช่วงวันที่จากคำถาม (เช่น "วันที่ 1 เดือน 6 ถึง 10 เดือน 6", "เดือนมิถุนายน") → scope ข้อมูลให้ตรง
  const { dateFrom, dateTo } = extractDateRange(userMessage, new Date().getFullYear());
  const dateWhere = buildDateWhere(dateFrom, dateTo);
  if (dateFrom || dateTo) {
    const dateNote = `> ขอบเขตข้อมูล: ${dateFrom ?? 'เริ่มต้น'} ถึง ${dateTo ?? 'ล่าสุด'}`;
    scopeNote = scopeNote ? `${scopeNote}\n${dateNote}` : dateNote;
  }

  const data = await getAggregatedData(fullName, dateWhere);

  // ถ้าถามเรื่องชั่วโมง/เวลาทำงานจริง → คำนวณอันดับชั่วโมงทำงานให้แล้วฉีดเข้า context (LLM ไม่ต้องคิดเลขเอง)
  const wantsWorkDuration = WORK_DURATION_KEYWORDS.some((kw) => userMessage.includes(kw));
  const workSection = wantsWorkDuration
    ? buildWorkSection(await getWorkDurationStats(fullName, dateWhere))
    : '';
  ...
  const contextMessage = buildContext(data, userMessage, scopeNote, '', workSection);
  ...
  if (selected === 'claude') {
    ...
    const claudeContext = buildContext(data, userMessage, scopeNote, buildHistorySection(prior), workSection);
